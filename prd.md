...
As 4 tabelas fonte estao no schema `DM AI` do PostgreSQL. Sao referenciadas via `{{ source('raw', 'nome_tabela') }}`.
...
Criar em `models/_sources.yml`. Definir as 4 tabelas fonte com source name `raw`, schema `DM AI`, e documentar todas as colunas de cada tabela conforme descrito na secao "Tabelas Fonte".
